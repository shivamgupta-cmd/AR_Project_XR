	...
	SubShader
	{
		...
		Pass
		{
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				float2 uv_PressureTexture = IN.ase_texcoord8.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float op62 = saturate( ( saturate( ( saturate( ( saturate( ( ( ( tex2DNode53.r * ( lerpResult250 * tex2D( _PressureTexture, ( float3( ( ( texCoord1_g21 * appendResult4_g21 ) + appendResult5_g21 ) ,  0.0 ) + float3( ( ( ( (tex2D( _PressureDistortionTextureNormal, panner70 ).rgb).xy + -0.5 ) * 2.0 ) * lerpResult266 ) ,  0.0 ) ).xy ).g ) ) * _OpacityMult ) * distanceDepth36 ) ) * ( 1.0 - fresnelNode1_g22 ) ) ) * saturate( cameraDepthFade111 ) ) ) * IN.ase_color.a ) );
		...
		Pass
		{
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float2 uv_PressureTexture = IN.ase_texcoord3.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float3 lerpResult235 = lerp( float3(0,0,1) , tex2D( _PressureTextureNormal, panner70 ).rgb , _NormalIntensity);
				float3 norm100 = lerpResult235;
				float2 uv_PressureTexture = IN.ase_texcoord5.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				...
				float2 appendResult4_g21 = (float2(1.0 , 1.0));
				float2 appendResult5_g21 = (float2(0.0 , 0.0));
				float2 texCoord262 = IN.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult266 = lerp( _UVNoiseStrength , 0.0 , saturate( ( saturate( texCoord262.y ) - 0.5 ) ));
				float screenDepth36 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth36 = saturate( ( screenDepth36 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade ) );
				float3 ase_viewVectorWS = ( _WorldSpaceCameraPos.xyz - WorldPosition );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace) : SV_TARGET
			{
				...
				float2 uv_PressureTexture = IN.ase_texcoord.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
...
ASEEND*/
//CHKSM=B790B09F6E9610F1D2F77E6157CDF59532BEC197